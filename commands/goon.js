...
export default {
  ...
  async execute({ channelName, senderUsername, msg, botState }) {
    ...
    // Upsert: new user starts at 1 goon, existing user gets +1. Keyed on
    // user_id (not username) so renames don't split someone's total.
    await new Promise((resolve, reject) => {
      botState.db.run(
        `INSERT INTO goon (user_id, username, goon) VALUES (?, ?, 1)
         ON CONFLICT(user_id) DO UPDATE SET goon = goon + 1, username = excluded.username`,
    ...
    const total = await new Promise((resolve, reject) => {
      botState.db.get(
        `SELECT goon FROM goon WHERE user_id = ?`,
        [userId],
        (err, row) => (err ? reject(err) : resolve(row ? row.goon : 1))
      );
    });

    const response = `@${senderUsername} has gooned, they have gooned ${total} times ꕥ`; // customize me
    await botState.client.me(channelName, response);
  },
};
